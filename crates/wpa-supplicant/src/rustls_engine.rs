...
impl TlsEngine for RustlsTlsEngine {
    ...
    fn derive_msk(&mut self) -> Result<pae::Msk, EapError> {
        ...
        // Key_Material derivation is version-scoped (#179 / F-EAP-4):
        //
        // * TLS 1.3 — RFC 9190 §2.3: Key_Material =
        //   TLS-Exporter("EXPORTER_EAP_TLS_Key_Material", Type, 128),
        //   the EAP Type code (0x0D) as exporter *context*.
        // * TLS 1.2 — RFC 5216 §2.3: Key_Material =
        //   PRF(MS, "EAP TLS Keying Material",
        //   client_random || server_random). That is exactly the
        //   RFC 5705 exporter with the same label and an EMPTY
        //   context, so rustls's `export_keying_material` computes it
        //   directly — no custom PRF needed.
        //
        // In both cases the first 64 bytes are the MSK and the next 64
        // the EMSK. The `Msk` type enforces >= 64 bytes at construction.
        let (label, context): (&[u8], Option<&[u8]>) = match conn.protocol_version() {
            // TLS 1.2 per RFC 5216 §2.3 (label + empty context).
            Some(rustls::ProtocolVersion::TLSv1_2) => (b"EAP TLS Keying Material", None),
            // TLS 1.3 per RFC 9190 §2.3 (and the fallback for any
            // other / unknown version, keeping historical behavior).
            _ => (
        ...
        conn.export_keying_material(&mut key_material, label, context)
...
mod tests {
    ...
    /// Verifies: #179 (REQ-F-EAP-002) — F-EAP-4
    /// Per RFC 5216 §2.3: under a TLS 1.2-negotiated session the MSK is
    /// `PRF(MS, "EAP TLS Keying Material", client_random || server_random)`
    /// — NOT the TLS 1.3 exporter label of RFC 9190 §2.3. The engine
    /// must branch on the negotiated version. Cross-checked against the
    /// server side of the loopback handshake (same RFC 5705 exporter
    /// with empty context), and asserted to differ from the TLS 1.3
    /// label so the branch cannot silently no-op.
    #[test]
    fn test_derive_msk_tls12_uses_rfc5216_prf() {
        let (server_der, server_key_pkcs8, ca_pem) = loopback_cert();

        // Server forces TLS 1.2 (no TLS 1.3 in the offered versions).
        ...
        assert!(
            engine.is_handshake_complete(),
            "TLS 1.2 handshake must complete"
        );
        assert_eq!(
            engine.conn.as_ref().and_then(|c| c.protocol_version()),
            Some(rustls::ProtocolVersion::TLSv1_2),
            "server must have forced TLS 1.2"
        );

        let msk = engine
            .derive_msk()
            .expect("TLS 1.2 MSK export must succeed");
        assert!(msk.len() >= 64);

        // Server-side reference: RFC 5705 exporter with EMPTY context is
        // PRF(MS, label, client_random || server_random) — the exact
        // RFC 5216 §2.3 Key_Material derivation.
        let mut server_km = vec![0u8; 128];
        server
            .export_keying_material(&mut server_km, b"EAP TLS Keying Material", None)
            .expect("server RFC 5216 2.3 export");
        assert_eq!(
            msk.as_bytes(),
            &server_km[..64],
            "TLS 1.2 MSK must match the RFC 5216 2.3 PRF derivation"
        );

        // The TLS 1.3 label must NOT reproduce the TLS 1.2 MSK — proves
        // the version branch actually switched formulas.
        let mut tls13_km = vec![0u8; 64];
        engine
            .conn
            .as_ref()
            .unwrap()
            ...
            .expect("TLS 1.3-label export over the same session");
        assert_ne!(
            msk.as_bytes(),
            tls13_km.as_slice(),
            "TLS 1.2 MSK must not equal the TLS 1.3 exporter derivation"
        );
    }
