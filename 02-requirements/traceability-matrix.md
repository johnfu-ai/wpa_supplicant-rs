...
| REQ-F-EAP-002 EAP-TLS | #39 | `9d6ffb9` | `crates/eap-peer/src/eap_tls.rs`, `crates/wpa-supplicant/src/method_factory.rs`, `…/rustls_engine.rs` | 19 dedicated tests + 6 factory/engine tests + `tests/method_factory.rs` | Implemented unit + bridge wired (#130 PR #134) + **method factory landed** (#133: PEM-loaded rustls engine drives a real TLS handshake validated by a loopback test; `Supplicant::new` wires it in). **Session-Id completed** (#174 / F-EAP-1: `Session-Id = 0x0D \|\| TLS session identifier` per RFC 5216 §1.4 / RFC 9190 §2.3 — exporter-derived Method-Id under TLS 1.3; TLS 1.2 falls back to the type byte alone, rustls 0.23 limitation). **Fragmentation/reassembly landed** (#175 / F-EAP-2, PR #181, RFC 5216 §3.1: inbound M-flag reassembly with announced-length validation + hard cap, outbound flight splitting at `TLS_MAX_FRAGMENT_LEN`, RFC 3748 §4.1 retransmission idempotence). **TLS 1.2 MSK derivation corrected** (#179 / F-EAP-4: `derive_msk` version-scoped — TLS 1.2 uses `PRF(MS, "EAP TLS Keying Material", client_random \|\| server_random)` per RFC 5216 §2.3). Live FreeRADIUS handshake validation is follow-up F-INT-1 (`docs/IMPROVEMENTS.md`). |
