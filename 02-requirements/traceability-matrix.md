...
| REQ-F-EAP-002 EAP-TLS | #39 | `b39c99e` | `crates/eap-peer/src/eap_tls.rs`, `crates/wpa-supplicant/src/method_factory.rs`, `…/rustls_engine.rs` | 13 dedicated tests + 6 factory/engine tests + `tests/method_factory.rs` | Implemented unit + bridge wired (#130 PR #134) + **method factory landed** (#133: PEM-loaded rustls engine drives a real TLS handshake validated by a loopback test; `Supplicant::new` wires it in). **Session-Id completed** (#174 / F-EAP-1: `Session-Id = 0x0D \|\| TLS session identifier` per RFC 5216 §1.4 / RFC 9190 §2.3 — exporter-derived Method-Id under TLS 1.3; TLS 1.2 falls back to the type byte alone, rustls 0.23 limitation). Live FreeRADIUS handshake validation is follow-up F-INT-1 (`docs/IMPROVEMENTS.md`). |
